...
export default function Step4Content({ wizardData, update }) {
  const [isGenerating, setIsGenerating] = useState(false);
  const { title, description } = wizardData.content;

  const handleGenerate = async () => {
    if (!wizardData.type) {
      Alert.alert('Missing info', 'Please complete Step 1 (type & category) first.');
      return;
    }
    setIsGenerating(true);
    try {
      const colors = [...new Set(wizardData.images.map((i) => i.color))];
      const sizes = wizardData.variants.map((v) => v.size).filter(Boolean);
      const result = await generateContent({
        productType: wizardData.type,
        category: wizardData.categoryId,
        colors,
        sizes,
      });
